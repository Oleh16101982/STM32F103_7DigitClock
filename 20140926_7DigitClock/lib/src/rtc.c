...
{
	rtcBCD->year 	= HEX8_to_BCD(rtc->year - FIRSTYEAR);
	rtcBCD->month = HEX8_to_BCD(rtc->month);
	rtcBCD->mday 	= HEX8_to_BCD(rtc->mday);
	rtcBCD->wday 	= HEX8_to_BCD(rtc->wday);
	rtcBCD->hour 	= HEX8_to_BCD(rtc->hour);
	rtcBCD->min 	= HEX8_to_BCD(rtc->min);
	rtcBCD->sec 	= HEX8_to_BCD(rtc->sec);
	rtcBCD->dst 	= HEX8_to_BCD(rtc->dst);	
}	
